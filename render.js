...
function renderQuestion(selected){
    ...
    createLineText([1100,Questionpadding, 100,Questionsize],(questionindex+1)+"/"+Qs.length).attr('fill',"lightgrey")
    SIZE = 1000
    Q = []
    leftbox.attr('y',-1000)
...
function renderInfoBox(box, result) {
    ...
    createLineText([box[0]+10, box[1]+10, box[2]-ResultPercentWidth+10, ResultPercentHeight], (result[1]/10)+"% "+LANGUAGES[result[0]][curlang]).attr('fill', result[0]=="nota"?"white":"black")
    createText([box[0]+10, box[1]+20+ResultPercentHeight, box[2]-ResultPercentWidth+10, box[3]-30-ResultPercentHeight], result[0]+"desc",0,result[0]=="nota"?"white":"black")
}
